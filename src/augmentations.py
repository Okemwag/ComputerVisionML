...
class MedicalAugmentations:
    ...
    def _create_augmentation_pipeline(self):
        ...
        try:
            ...
            if self.gaussian_noise > 0:
                transforms.append(
                    A.GaussNoise(var_limit=(0, self.gaussian_noise), p=0.3)
                )
            ...
            transforms.extend(
                [
                    ...
                    A.ShiftScaleRotate(
                        shift_limit=0.05, scale_limit=0, rotate_limit=0, p=0.3
                    ),
            ...
            self.augmentation_pipeline = A.Compose(
                transforms, p=self.augmentation_probability
            )
    ...
    def _create_torch_pipeline(self):
        ...
        if self.brightness_limit > 0 or self.contrast_limit > 0:
            transforms.append(
                T.ColorJitter(
                    brightness=self.brightness_limit, contrast=self.contrast_limit
                )
            )
...
def create_medical_transforms(config: Dict[str, Any]) -> MedicalAugmentations:
    ...
    return MedicalAugmentations(
        ...
        augmentation_probability=augmentation_config.get(
