...
class ModelInference:
    ...
    def predict_single(self, image_path: str, return_probabilities: bool = True) -> Dict[str, Any]:
        """
    ...
    def predict_batch(self, image_paths: List[str], batch_size: int = 16, return_probabilities: bool = True) -> List[Dict[str, Any]]:
        """
    ...
    def _process_batch(self, image_paths: List[str], return_probabilities: bool = True) -> List[Dict[str, Any]]:
            ...
            for i, (tensor_idx, meta) in enumerate(zip(valid_indices, batch_metadata)):
                if 'error' not in meta:
                    ...
                    if isinstance(batch_outputs, dict):
                        single_output = {k: v[i:i+1] for k, v in batch_outputs.items()}
                    else:
    ...
    def _process_outputs(self, outputs: Union[torch.Tensor, Dict[str, torch.Tensor]], return_probabilities: bool = True) -> Dict[str, Any]:
        ...
        if isinstance(outputs, dict):
            ...
            if 'classification' in outputs:
                ...
                if return_probabilities:
                    results['classification']['probabilities'] = probabilities.cpu().numpy().tolist()
            ...
            if 'segmentation' in outputs:
                ...
                results['segmentation'] = {
                    ...
                    'confidence_map': seg_probabilities.max(dim=1)[0].cpu().numpy().tolist()
                }
                
                if return_probabilities:
                    results['segmentation']['probability_maps'] = seg_probabilities.cpu().numpy().tolist()
            ...
            if task_type == 'classification':
                ...
                results = {
                    ...
                    'confidence_map': probabilities.max(dim=1)[0].cpu().numpy().tolist()
                }
...
class BatchProcessor:
    ...
    def __init__(self, model: torch.nn.Module, batch_size: int = 16, device: str = 'cuda'):
        """
    ...
    def process_directory(self, input_dir: str, output_dir: str, file_extensions: List[str] = ['.dcm', '.tif', '.tiff']) -> Dict[str, Any]:
        """
        ...
        summary = {
            ...
            'processed_successfully': len([r for r in results if r.get('processed', False)]),
    ...
    def process_with_metadata(self, image_paths: List[str], metadata: pd.DataFrame) -> pd.DataFrame:
        """
...
def create_inference_engine(model_path: str, config_path: str, device: str = 'cuda') -> ModelInference:
    """
...
def run_inference_from_cli(model_path: str, config_path: str, input_path: str, output_path: str, batch_size: int = 16) -> Dict[str, Any]:
    """
