...
from .metrics import CheckpointManager, MedicalMetrics
...
class ModelEvaluator:
    ...
    def evaluate_dataset(
        self, dataloader: torch.utils.data.DataLoader, dataset_name: str = "test"
    ) -> Dict[str, Any]:
        ...
        metrics = self.medical_metrics.compute_classification_metrics(
            torch.from_numpy(all_predictions), torch.from_numpy(all_targets)
        )
    ...
    def cross_validate(
        self, dataset: MedicalImageDataset, k_folds: int = 5
    ) -> Dict[str, Any]:
        ...
        return {"cv_metrics": cv_results, "fold_results": fold_results}
    ...
    def save_results(self, output_dir: str = "outputs/evaluation"):
        ...
        for dataset_name in self.evaluation_results.keys():
            ...
            predictions_df = pd.DataFrame(
                {
                    ...
                    "probabilities_class_0": self.predictions_cache[dataset_name][
                        "probabilities"
                    ][:, 0],
                    "probabilities_class_1": (
                        self.predictions_cache[dataset_name]["probabilities"][:, 1]
                        if self.predictions_cache[dataset_name]["probabilities"].shape[
                            1
                        ]
                        > 1
                        else 0
            ...
            predictions_df.to_csv(
                output_path / f"{dataset_name}_predictions.csv", index=False
            )
...
def create_evaluator(
    config: Dict[str, Any], checkpoint_path: Optional[str] = None, device: str = "cuda"
) -> ModelEvaluator:
    """
...
def evaluate_model_from_config(
    config_path: str, checkpoint_path: str, data_dir: str, metadata_file: str
) -> Dict[str, Any]:
    """
