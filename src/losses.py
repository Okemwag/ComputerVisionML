...
class DiceLoss(nn.Module):
    ...
    def forward(self, predictions: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        ...
        if targets.dim() == predictions.dim() - 1:
            targets = (
                F.one_hot(targets, num_classes=predictions.size(1))
                .permute(0, -1, *range(1, targets.dim()))
                .float()
            )
...
class CombinedLoss(nn.Module):
    ...
    def forward(
        ...
        # Combined loss
        total_loss = (
            self.classification_weight * cls_loss + self.segmentation_weight * seg_loss
        )
...
class LossManager:
    ...
    def _combined_segmentation_loss(
    ...
    def compute_loss(self, predictions, targets, **kwargs) -> torch.Tensor:
        ...
        if self.task_type in ["classification", "segmentation"]:
            ...
            return self.criterion(
                cls_predictions, seg_predictions, cls_targets, seg_targets
            )


def create_loss_function(
    task_type: str, config: Dict[str, Any], class_weights: Optional[torch.Tensor] = None
):
    """
