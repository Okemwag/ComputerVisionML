    ...
    def from_dicom(cls, dicom_path: str, metadata: Dict[str, Any]) -> "MedicalImage":
        ...
        try:
            ...
            raise DataLoadingError(
                f"Failed to create MedicalImage from DICOM {dicom_path}: {str(e)}"
            )
...
class DICOMLoader:
    ...
    def _extract_dicom_metadata(self, dicom_data) -> Dict[str, Any]:
        ...
        metadata["image_height"] = (
            dicom_data.Rows if hasattr(dicom_data, "Rows") else None
        )
        metadata["image_width"] = (
            dicom_data.Columns if hasattr(dicom_data, "Columns") else None
        )
        metadata["bits_allocated"] = (
            dicom_data.BitsAllocated if hasattr(dicom_data, "BitsAllocated") else None
        )
...
class TIFFLoader:
    ...
    def load_tiff(self, tiff_path: str) -> Tuple[torch.Tensor, Dict[str, Any]]:
        ...
        try:
            ...
            if pixel_array.size == 0:
                raise DataLoadingError(
                    f"Loaded empty array from TIFF file: {tiff_path}"
                )
    ...
    def _extract_tiff_metadata(
        self, tiff_path: str, image_shape: Tuple[int, ...]
    ) -> Dict[str, Any]:
...
class ImageLoader:
        ...
        try:
            if file_extension == ".dcm":
                ...
                try:
                    ...
                    return self._create_medical_image_from_tiff(
    ...
    def _create_medical_image_from_tiff(
        self, tiff_path: str, metadata: Dict[str, Any]
    ) -> MedicalImage:
        ...
        try:
            ...
            raise DataLoadingError(
                f"Failed to create MedicalImage from TIFF {tiff_path}: {str(e)}"
            )

    def load_batch(
        self, image_paths: list, metadata_list: Optional[list] = None
    ) -> list:
        ...
        if failed_loads:
            self.logger.info(
                f"Successfully loaded {len(medical_images)}/{len(image_paths)} images"
            )
