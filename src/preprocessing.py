...
class ImagePreprocessor:
    ...
    def __init__(
        self, target_size: Tuple[int, int] = (256, 256), normalize: bool = True
    ):
    ...
    def normalize_to_window(
        self, image_tensor: torch.Tensor, window_type: str = "soft_tissue"
    ) -> torch.Tensor:
...
class MedicalImagePreprocessor:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.preprocessor = ImagePreprocessor(
            target_size=tuple(self.image_size), normalize=True
        )
        self.logger = logging.getLogger(self.__class__.__name__)
    ...
    def preprocess_medical_image(self, medical_image: MedicalImage) -> MedicalImage:
        ...
        try:
            if medical_image.image_format == "dicom":
                ...
                processed_tensor = self.preprocessor._preprocess_tensor(
            ...
            raise DataLoadingError(
                f"Failed to preprocess medical image {medical_image.image_id}: {str(e)}"
            )

    def _preprocess_with_metadata(
        self, image_tensor: torch.Tensor, metadata: Dict[str, Any]
    ) -> torch.Tensor:
        ...
        if self.normalize_method == "window":
            image_tensor = self.preprocessor.normalize_to_window(
                image_tensor, self.window_type
            )
        ...
        if tuple(self.image_size) != image_tensor.shape[-2:]:
            image_tensor = self.preprocessor._resize_tensor(
                image_tensor, tuple(self.image_size)
            )
