...
def test_training_curves():
    ...
    try:
        ...
        metrics_history = {
            'train': {
                'loss': [1.5 - 0.05*i + 0.1*np.random.random() for i in range(epochs)],
                'accuracy': [60 + 2*i + 5*np.random.random() for i in range(epochs)],
                'f1': [0.6 + 0.02*i + 0.05*np.random.random() for i in range(epochs)]
            },
            'val': {
                'loss': [1.4 - 0.04*i + 0.15*np.random.random() for i in range(epochs)],
                'accuracy': [62 + 1.8*i + 8*np.random.random() for i in range(epochs)],
                'f1': [0.62 + 0.018*i + 0.08*np.random.random() for i in range(epochs)]
            },
...
def test_confusion_matrix():
    ...
    try:
        ...
        viz_engine.plot_confusion_matrix(confusion_matrix, class_names, 'test_confusion_matrix.png')
...
def test_roc_curves():
    ...
    try:
        ...
        viz_engine.plot_roc_curves(targets, probabilities, class_names, 'test_roc_curves.png')
...
def test_prediction_visualization():
    ...
    try:
        ...
        probabilities_tensor = torch.tensor([
            [0.3, 0.7] if pred == 1 else [0.8, 0.2] 
            for pred in predictions_tensor
        ]).float()
        
        viz_engine.visualize_predictions(
            images_tensor, predictions_tensor, targets_tensor, 
            probabilities_tensor, num_samples=8, save_name='test_predictions.png'
        )
...
def test_model_comparison():
    ...
    try:
        ...
        viz_engine.plot_model_comparison(comparison_results, 'test_model_comparison.png')
...
def test_segmentation_overlay():
    ...
    try:
        ...
        plt.tight_layout()
        plt.savefig(viz_engine.save_dir / 'test_segmentation_overlay.png', dpi=300, bbox_inches='tight')
        plt.close()
